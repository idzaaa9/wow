...
public:
  ...
  Mob(const Character& refCharacter, const std::map<item, float>& refLootTable, Agressivity refAgr, const stats& refStats) :
	  Character(refCharacter), lootTable(refLootTable), mobAgr(refAgr), mobStats(refStats) {}
  Mob(const Mob& ref) :
	  Character(ref.getLevel(), ref.getListSkill(), ref.getPosition(), ref.getModel(), ref.getFac(), ref.getName()), lootTable(ref.getLootTable()),
	  mobAgr(ref.getAgr()), mobStats(ref.getMobStats()){}
